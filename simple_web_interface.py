#!/usr/bin/env python3
"""
Simple web interface for Crystal Archive - Standalone version
Run with: python3 simple_web_interface.py
"""
...
class CrystalArchiveHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            ...
            self.end_headers()
            self.wfile.write(self.run_demo().encode())
        else:
            self.send_error(404)
    ...
    def handle_encode(self):
        try:
            ...
            # Get profile
            profile = form_data.get('profile', ['A'])[0]
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.send_header('Content-Disposition', 'attachment; filename="crystal_archive.zip"')
                self.end_headers()
            ...
            self.send_error(500, f"Encoding failed: {str(e)}")
    ...
    def handle_decode(self):
        try:
            ...
            self.send_error(500, f"Decoding failed: {str(e)}")
    
    def run_demo(self):
        try:
            # Run the actual crystal-archive demo
...
def main():
    ...
    for port in ports_to_try:
        try:
            server_address = ('', port)
            httpd = HTTPServer(server_address, CrystalArchiveHandler)
